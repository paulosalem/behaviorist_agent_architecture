...
/**
 * Activates a response, once its latency reaches zero.
 * 
 * @author Paulo Salem
 *
 */
public class InactiveResponseUpdateOp_2 extends Operation {
  ...
  public boolean precondition(CurrentResponses currResp, Response resp){
    ...
    if(currResp.getInactiveResponses().contains(resp)
        && (!currResp.getActiveResponses().contains(resp))
        && resp.getLatency() <= 0){
  ...
  public void apply(CurrentResponses currResp, Response resp){
    ...
    currResp.removeInactiveResponse(resp);
    currResp.addActiveResponse(resp);
  }
}
